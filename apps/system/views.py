...
class PTaskViewSet(OptimizationMixin, ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        clocked_ = data.get('clocked_', None)
        ...
        if timetype == 'clocked' and clocked_:
            data['crontab'] = None
            data['interval'] = None

            try:
                clocked, _ = ClockedSchedule.objects.get_or_create(**clocked_, defaults=clocked_)
...
class UserViewSet(ModelViewSet):
    ...
    @action(methods=['put'], detail=False, permission_classes=[IsAuthenticated], perms_map={'put': 'change_password'},
            url_name='change_otherspassword')
    def change_password(self, request, *args, **kwargs):
        """
        修改密码
        """
        user = User.objects.filter(pk=request.data.get('id', None)).first()
        if not user:
