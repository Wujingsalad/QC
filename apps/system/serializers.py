...
from django_celery_beat import clockedschedule
...
class PTaskCreateUpdateSerializer(serializers.ModelSerializer):
    class Meta:
        model = PeriodicTask
        fields = ['name', 'task', 'interval', 'crontab', 'one_off', 'clocked', 'args', 'kwargs']
...
class PTaskSerializer(serializers.ModelSerializer):
    ...
    clocked_ = ClockedSerializer(source='clocked', read_only=True)
